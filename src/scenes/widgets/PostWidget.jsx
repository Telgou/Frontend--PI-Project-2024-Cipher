...
const PostWidget = ({
  ...
  const patchLike = async () => {
    const response = await fetch(`http://backend-pi-project-2024-cipher.onrender.com/posts/${postId}/like`, {
      method: "PATCH",
      headers: {
  ...
  const CommentSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://backend-pi-project-2024-cipher.onrender.com/posts/${postId}/comments`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
            ...
            <div style={{ display: 'flex', marginTop: '1rem' }} key={`${name}-${i}`}>
              ...
              <Box width={'60px'} height={'60px'}>
                <img
                  ...
                  height={'40rem'}
                  alt="user"
                  src={`http://backend-pi-project-2024-cipher.onrender.com/assets/${comment.userpic}`}
                ></img>
              </Box>
